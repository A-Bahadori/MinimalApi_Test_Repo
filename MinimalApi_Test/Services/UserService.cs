...
namespace MinimalApi_Test.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<ResultCustom<UserDto>> CreateUserAsync(CreateUserDto createUserDto,
            CancellationToken cancellationToken)
        {
            using (MiniProfiler.Current.Step("CreateUserAsync"))
            {
                using (MiniProfiler.Current.Step("Validate CreateUserDto"))
                {
                ...
                try
                {
                    using (MiniProfiler.Current.Step("Database Transaction"))
                    {
                        await _userRepository.BeginTransactionAsync(cancellationToken);

                        using (MiniProfiler.Current.Step("Check Username Existence"))
                        {
                            var exists = await _userRepository.ExistsAsync(u => u.Username == createUserDto.Username,
                                cancellationToken);
                            if (exists)
                            {
        ...
        public async Task<ResultCustom<UserDto>> UpdateUserAsync(int id, UpdateUserDto updateUserDto,
            CancellationToken cancellationToken)
        {
            using (MiniProfiler.Current.Step("UpdateUserAsync"))
            {
                using (MiniProfiler.Current.Step("Validate UpdateUserDto"))
                {
                ...
                try
                {
                    using (MiniProfiler.Current.Step("Database Transaction"))
                    {
                        ...
                        User? user;
                        using (MiniProfiler.Current.Step("Get User"))
                        {
                            user = await _userRepository.GetByIdAsync(id, tracking: true,
                                cancellationToken: cancellationToken);
                            if (user == null)
                            {
                                await _userRepository.RollbackTransactionAsync(cancellationToken);
                        ...
                        using (MiniProfiler.Current.Step("Check Username Uniqueness"))
                        {
                            if (!string.IsNullOrEmpty(updateUserDto.Username) &&
                                updateUserDto.Username != user.Username)
                            {
                                var exists =
                                    await _userRepository.ExistsAsync(u => u.Username == updateUserDto.Username,
                                        cancellationToken);
                                if (exists)
                                {
                                    await _userRepository.RollbackTransactionAsync(cancellationToken);
                        ...
                        using (MiniProfiler.Current.Step("Update User"))
                        {
        ...
        public async Task<ResultCustom<UserDto>> PatchUserAsync(int id, JsonPatchDocument<PatchUserDto> patchDoc,
            CancellationToken cancellationToken)
        {
            using (MiniProfiler.Current.Step("PatchUserAsync"))
            {
                ...
                try
                {
                    using (MiniProfiler.Current.Step("Database Transaction"))
                    {
                        ...
                        var user = await _userRepository.GetByIdAsync(id, tracking: true,
                            cancellationToken: cancellationToken);
                        if (user == null)
                        {
                            await _userRepository.RollbackTransactionAsync(cancellationToken);
                        ...
                        {
                            modelState.TryAddModelError(patchError.AffectedObject?.ToString() ?? string.Empty,
                                patchError.ErrorMessage);
                        }
                        ...
                        if (patchDoc.Operations.Any(op =>
                                op.path.Contains("/Username", StringComparison.OrdinalIgnoreCase)))
                        {
                            var exists =
                                await _userRepository.ExistsAsync(u => u.Username == user.Username && u.Id != id,
                                    cancellationToken);
                            if (exists)
                            {
                                await _userRepository.RollbackTransactionAsync(cancellationToken);
        ...
        {
            using (MiniProfiler.Current.Step("DeleteUserAsync"))
            {
                try
                {
                    using (MiniProfiler.Current.Step("Database Transaction"))
        ...
        {
            using (MiniProfiler.Current.Step("GetUserByIdAsync"))
            {
                try
                {
                    var user = await _userRepository.GetByIdAsync(id, tracking: false,
                        cancellationToken: cancellationToken);
                    if (user == null)
                    {
                        return ResultCustom<UserDto>.Failure("User not found");
        ...
        {
            using (MiniProfiler.Current.Step("GetAllUsersAsync"))
            {
                try
                {
                    IReadOnlyList<User>? users;

                    using (MiniProfiler.Current.Step("Repository.GetAsync"))
                    {
                        users = await _userRepository.GetAsync(specification: null, tracking: false,
                            cancellationToken: cancellationToken);
                    }

                    using (MiniProfiler.Current.Step("Mapper.Map"))
                    {
        ...
        public async Task<ResultCustom<(bool IsValid, UserDto User)>> ValidateCredentialsAsync(string username,
            string password, CancellationToken cancellationToken)
        {
            using (MiniProfiler.Current.Step("ValidateCredentialsAsync"))
            {
                try
                {
                    var user = (await _userRepository.GetAsync(u => u.Username == username, tracking: false,
                        cancellationToken: cancellationToken)).FirstOrDefault();

                    if (user == null)
                    {
                        return ResultCustom<(bool, UserDto)>.Failure("Invalid credentials");
                    }

                    var result = _passwordHasher.VerifyHashedPassword(user, user.PasswordHash, password);
                    var userDto = _mapper.Map<UserDto>(user);

                    return ResultCustom<(bool, UserDto)>.Success(
                        (result == PasswordVerificationResult.Success, userDto));
                }
                catch (Exception ex)
                {
                    return ResultCustom<(bool, UserDto)>.Failure($"Error validating credentials: {ex.Message}");
                }
            }
        }

        public async Task<ResultCustom<SearchUsersResult>> SearchUsersAsync(SearchUserDto searchDto,
            CancellationToken cancellationToken)
        {
            using (MiniProfiler.Current.Step("SearchUsersAsync"))
            {
                try
                {
                    ...
                    {
                        specification =
                            specification.And<User>(u => u.Username.ToLower() == searchDto.Username.ToLower());
                    }

                    if (!string.IsNullOrWhiteSpace(searchDto.Role))
                    {
                        specification = specification.And<User>(u => u.Role.ToLower() == searchDto.Role.ToLower());
                    }

                    if (searchDto.CreatedFromDate.HasValue)
                    {
                        specification = specification.And<User>(u => u.CreatedAt >= searchDto.CreatedFromDate.Value);
                    }

                    if (searchDto.CreatedToDate.HasValue)
                    {
                        specification = specification.And<User>(u => u.CreatedAt <= searchDto.CreatedToDate.Value);
                    }
                    ...
                    if (searchDto.IsDeleted ?? false)
                    {
                        ...
                        users = await _userRepository.GetWithDeletedAsync(specification: specification,
                            orderBy: orderBy, tracking: false, cancellationToken: cancellationToken);
                        users = users.Where(u => u.IsDelete).ToList();
                    }
                    else
                    {
                        totalCount = await _userRepository.CountAsync(specification, cancellationToken);
                        users = await _userRepository.GetAsync(specification: specification, orderBy: orderBy,
                            tracking: false, cancellationToken: cancellationToken);
                    }

                    var pagedUsers = users.Skip((searchDto.PageNumber - 1) * searchDto.PageSize)
                        .Take(searchDto.PageSize).ToList();
                    var searchUserResult = new SearchUsersResult(_mapper.Map<List<UserDto>>(pagedUsers), totalCount,
                ...
                {
                    return ResultCustom<SearchUsersResult>.Failure($"Error searching users: {ex.Message}");
                }
            }
        }
    }
